...
- **The tmux bar is quiet until a session needs you**: `ccbar.py` shows nothing unless a
  ...
  acked/dismissed, not **focused**; `engaged` = Claude has produced ≥1 assistant turn, so a
  freshly opened never-answered idle session isn't a false "needs you"; `focused` = the pane
  you're watching live, which never soft-alerts — see "Seen = handled" below) —
...
- **Five states**: `blocked`, `busy`, `shell`, `idle`, `dead` (mapped back to
  ...
  is also `engaged` (Claude produced ≥1 assistant turn), not acked/dismissed, and not `focused`
